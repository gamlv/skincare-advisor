...
スマホで `https://192.168.50.6:5173` を開く（IPは手順1で確認した値に変える）。

> **注意：** バーコードスキャン機能はカメラAPIの制約により **HTTPS** が必要。
> 開発サーバーは `@vitejs/plugin-basic-ssl` により自動でHTTPSになる。
> 初回アクセス時にブラウザの「安全でない接続」警告が出るので「詳細設定」→「アクセスする」で許可する。
...
| バーコードスキャン | html5-qrcode + Open Beauty Facts API |
| 天気情報取得 | ブラウザ Geolocation API + Open-Meteo（無料・APIキー不要） |
...
| キー | 日本語 | 提案方針 |
|------|--------|---------|
| `morning` | 朝ケア | 日焼け止め必須・レチノール除外・軽めで化粧下地としても機能する製品を優先 |
| `night` | 夜ケア | 日焼け止め不要・美容液/クリームを厚めに・レチノールや高濃度成分も積極活用 |
...
| `brightening` | 美白ケア | ビタミンC・ナイアシンアミド・トラネキサム酸含む製品を優先 |
| `antiaging` | エイジングケア | レチノール・ペプチド・コラーゲン・セラミド重視・保湿もしっかり |
| `pore` | 毛穴ケア | BHA・ナイアシンアミド優先・皮脂コントロール・重ためオイル系除外 |

---

## 天気・環境コンテキスト連携

ルーティン提案ページでは、ブラウザの位置情報を使って自動的に天気情報を取得し、提案内容に反映する。

- **取得情報**：気温・湿度・UV指数・天気（晴れ/曇り/雨 等）
- **天気API**：[Open-Meteo](https://open-meteo.com/)（無料・APIキー不要）
- **花粉**：手動トグルで ON/OFF 設定
- **反映例**：湿度 40% 未満 → 保湿強化推奨、UV 高 → 日焼け止め必須、花粉 ON → バリアケア優先

`POST /routine/suggest` の `weather` フィールド（任意）で送信する：
