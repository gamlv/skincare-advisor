...
- **気分選択**：10種類（朝ケア・夜ケア・リフレッシュ・リラックス・しっかりケア・時短・肌荒れ・美白ケア・エイジングケア・毛穴ケア）を最大3つまで複数選択可能 ✅
- **天気・環境連携**：Geolocation + Open-Meteo で気温・湿度・UV・天気を自動取得 → Claude プロンプトに組み込む。花粉は手動トグル ✅
- **ルーティン提案**：選択した気分（複数可）・天気に合わせてClaude APIが最適な組み合わせを提案 ✅
...
## routine_suggester.py の重要な実装メモ

`suggest_routine(products, moods, weather)` で気分・天気に合わせたルーティンを提案する。

**気分の複数選択：**
- `moods: list[str]`（1〜3件）を受け取り、各モードの `policy` を箇条書きで Claude プロンプトに渡す
- 複数選択時は「すべての方針を考慮してバランスよく」という指示を末尾に付加する
- `mood_label` はラベルを `" + "` で結合して返す（例：`"朝ケア + 時短"`）

**天気コンテキスト：**
- `weather` dict（任意）が渡された場合のみプロンプトに追加する
- 湿度 40% 未満 → 乾燥注意の注釈付き
- UV指数は 0-2=低/3-5=中/6-7=高/8+=非常に高い の4段階でラベル化
- 花粉フラグが True の場合はバリアケア推奨の一文を追加
...
- [x] Step 8: 気分の複数選択対応（最大3つ・選択順バッジ表示・moods: list[str]）
